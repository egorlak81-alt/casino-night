...
def add_balance(tg_id, d):
    qexec("""UPDATE players SET balance=balance+%s,
        ...
        games_played=games_played+1 WHERE tg_id=%s""",(d,d,d,d,d,tg_id))
...
def _cleanup(r):
    ...
    if not r['players']:
        r.update({'host_id':None,'messages':[],'actions':[],'msg_seq':0,'act_seq':0,'countdown_start':None,'started':False})
    else:
        if r['host_id'] not in [p['id'] for p in r['players']]: r['host_id']=r['players'][0]['id']
...
def api_rooms_join():
    ...
    d=request.get_json() or {}; rid=d.get("room_id"); uid=d.get("uid"); name=d.get("name","Игрок")
